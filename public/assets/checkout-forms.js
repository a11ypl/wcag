window.A11yFirstCheckout = (() => {
    ...
    function getSelectedProducts(form) {
        ...
        if (selectedProducts.length) {
            return selectedProducts.map((selected) => ({
                ...
                category: selected.dataset.productCategory || '',
        ...
        if (form.dataset.productTitle) {
            return [{
                ...
                category: form.dataset.productCategory || '',
    ...
    function getCartSummary(products, form) {
        ...
        return {
            ...
            ids,
            titles,
            transferTitle: transferTitles.length > 1 ? `Produkty: ${transferTitles.join('; ')}` : transferTitles[0] || ''
        };
    }
    ...
    function validateProduct(form) {
        ...
        const products = getSelectedProducts(form);
        const message = products.length ? '' : 'Wybierz co najmniej jeden produkt, który chcesz kupić.';
