window.A11yFirstCheckout = (() => {
    ...
    function updateSummary(form) {
        ...
        if (totalOutput) {
            totalOutput.textContent = products.length ? `${cart.total} zł` : '0 zł';
        }
    }
    ...
    function validateField(field) {
        ...
        if (field.type === 'checkbox') {
            message = field.checked ? '' : `Zaznacz pole: ${label}.`;
    ...
    function fillTransferInfo(form, products, participant) {
        ...
        if (amount) amount.textContent = `${cart.total} zł`;
    ...
    async function submitForm(form) {
        ...
        payload.append('amount', `${cart.total} zł`);
